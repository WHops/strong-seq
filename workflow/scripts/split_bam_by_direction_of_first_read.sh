...
${samtools_path} view -f $include_flag -F $exclude_flag $input_bam | awk '{print $1}' > $readnames

# Extract BAM header
${samtools_path} view -H $input_bam > $header

# Extract read pairs corresponding to these names and append to header
${samtools_path} view $input_bam | grep -Fwf $readnames | cat $header - | ${samtools_path} view -bS - > $output_bam
