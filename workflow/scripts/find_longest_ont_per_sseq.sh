...
prefix_A=$(basename $bam_A .bam)_${chr}
prefix_B=$(basename $bam_B .bam)_${chr}_HP${hp}

# Convert BAM files to BEDPE format to represent paired-end reads in A
${bedtools_bin} bamtobed -bedpe -i $bam_A > "${prefix_A}_A.bedpe"
...
do
    ...
    if [[ -z "$overlaps" ]]; then
        echo -e "${pair[0]}\t$start1\t$end1\t${name}\t60\t$strand1\t$start2\t$end2\t${name}\t60\t$strand2" >> ${outfile}
    else
